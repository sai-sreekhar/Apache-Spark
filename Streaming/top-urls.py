...
accessLines = spark.readStream.text("./../assets/logs")
...
endpointCounts = logsDF2.groupBy(func.window(func.col("eventTime"), \
      "30 seconds", "10 seconds"), func.col("endpoint")).count().withColumnRenamed("count", "endpointCount")

sortedEndpointCounts = endpointCounts.orderBy(func.col("endpointCount").desc())

# Display the stream to the console
query = sortedEndpointCounts.writeStream.outputMode("complete").format("console").option("truncate", False) \
